...
static int expr()
{
	...
	case 'e':
		reg = next_register();
		char c = variable();
		CodeGen(LOAD, reg, c, EMPTY_FIELD);
		return reg;
...
static void assign()
{
	...
	case 'e':
		...
		next_token();
		exprReg = expr();
		CodeGen(STORE, c, exprReg, EMPTY_FIELD);
		return;
	default:
		ERROR("Symbol %c unknown\n", token);
		exit(EXIT_FAILURE);
...
static void read()
{
	...
	if(token == '!'){
		next_token();
		c = variable();
		CodeGen(READ,c, EMPTY_FIELD, EMPTY_FIELD);
	}else{
		ERROR("Symbol %c unknown\n", token);
		exit(EXIT_FAILURE);
	}
...
static void print()
{
	...
	if(token == '#'){
		next_token();
		c = variable();
		CodeGen(WRITE,c, EMPTY_FIELD, EMPTY_FIELD);
...
{
	/* YOUR CODE GOES HERE */
	//expr(); This was in the originally given code
